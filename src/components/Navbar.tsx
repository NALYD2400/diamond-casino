        ...
        <div className="flex items-center gap-3 sm:gap-4 pointer-events-auto">
          ...
          {isAuthenticated && user ? (
            <Link
              ...
              className={`liquid-glass flex items-center gap-3 text-white rounded-full pl-1.5 pr-4 sm:pr-5 py-1.5 hover:bg-white/10 transition-all cursor-pointer ${
                isMember ? 'ring-1 ring-white/60' : ''
              }`}
            >
              <img
                src={user.avatarUrl}
                alt={user.rpFirstName}
                className="w-8 h-8 sm:w-9 sm:h-9 rounded-full object-cover border-2 border-white shadow-[0_0_10px_rgba(255,255,255,0.25)]"
              />
