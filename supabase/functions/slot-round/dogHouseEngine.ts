...
/**
 * Multiplicateur du poids des scatters par rouleau en mode Boost.
 * Calibré par simulation (1,5 M de manches) pour garder RTP ≈ 95 % :
 * avec 1.28 (« bonus x2 ») le Boost rendait ≈ 105 %, donc perdant pour le casino.
 */
export const BOOST_SCATTER_MULTIPLIER = 1.22;
...
// =============================================================================
// Manche complète (tour de base + tours gratuits éventuels)
...
export type DogRoundMode = 'spin' | 'boost' | 'buy';

export interface DogFreeSpinsRound {
  /** Grille 3x3 tirée à l'intro (somme = nombre de tours) */
  values: number[];
  spins: DogSpinResult[];
  win: number;
}

export interface DogHouseRound {
  mode: DogRoundMode;
  bet: number;
  cost: number;
  base: DogSpinResult;
  freeSpins: DogFreeSpinsRound | null;
  totalWin: number;
}

export function dogRoundCost(bet: number, mode: DogRoundMode, buyPriceX: number = BONUS_BUY_X_BET): number {
  if (mode === 'buy') return Math.round(bet * buyPriceX * 100) / 100;
  if (mode === 'boost') return Math.round(bet * BOOST_BET_MULTIPLIER * 100) / 100;
  return bet;
}

export function playDogHouseRound(params: {
  bet: number;
  mode: DogRoundMode;
  buyPriceX?: number;
  ...
  if (base.triggersBonus) {
    const values = rollFreeSpinsGrid(rng);
    const total = values.reduce((a, b) => a + b, 0);
    const spins: DogSpinResult[] = [];
    let sticky: StickyWild[] = [];
    let win = 0;
    ...
    freeSpins = { values, spins, win };
  }

  return {
    mode,
    bet,
    cost: dogRoundCost(bet, mode, buyPriceX),
    base,
    freeSpins,
    totalWin: Math.round((base.totalWin + (freeSpins?.win ?? 0)) * 100) / 100,
  };
}
